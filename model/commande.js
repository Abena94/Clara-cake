 ...
 * et en vidant le panier dans la base de données. (Le panier de l'utilisateur connecté)
 */
export const addCommande = async (iduser) => {
    let connection = await connectionPromise;
    ...
    await connection.run(
        ...
        SELECT ?, id_produit, quantite FROM panier WHERE id_utilisateur=? `,
        [result.lastID, iduser]
    );

    // Vide le panier.
    await emptyPanier(iduser);
}
